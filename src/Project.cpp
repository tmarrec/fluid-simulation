...
inline double Project::pressureAt(const std::uint16_t i, const std::uint16_t j, const std::uint16_t k, const Eigen::VectorXd x, const std::uint64_t l) const
{
...
void Project3D::preparePressureSolving(Eigen::SparseMatrix<double>& A, Eigen::VectorXd& b)
{
    ...
    for (std::uint64_t n = 0; n < _grid._surface.maxIt(); ++n)
    {
        ...
        if (id > 0)
        {
            ...
            if (i+1 < _grid._surface.x() && (neibID = _grid._pressureID(i+1,j,k)) > 0)
            {
                A.coeffRef(realID, neibID-1) = -scale;
                _grid._Adiag(i,j,k) += scale;
                _grid._Ax(i,j,k) = -scale;
            }
            else if (i+1 < _grid._surface.x() && _grid._surface.label(i+1,j,k) & EMPTY)
            {
                _grid._Adiag(i,j,k) += scale;
            }
            ...
            if (j+1 < _grid._surface.y() && (neibID = _grid._pressureID(i,j+1,k)) > 0)
            {
                A.coeffRef(realID, neibID-1) = -scale;
                _grid._Adiag(i,j,k) += scale;
                _grid._Ay(i,j,k) = -scale;
            }
            else if (j+1 < _grid._surface.y() && _grid._surface.label(i,j+1,k) & EMPTY)
            {
                _grid._Adiag(i,j,k) += scale;
            }
            ...
            if (k+1 < _grid._surface.z() && (neibID = _grid._pressureID(i,j,k+1)) > 0)
            {
                A.coeffRef(realID, neibID-1) = -scale;
                _grid._Adiag(i,j,k) += scale;
                _grid._Az(i,j,k) = -scale;
            }
            else if (k+1 < _grid._surface.z() && _grid._surface.label(i,j,k+1) & EMPTY)
            {
                _grid._Adiag(i,j,k) += scale;
            }
...
inline double Project3D::div(const std::uint16_t i, const std::uint16_t j, const std::uint16_t k) const
{
...
void Project2D::project()
{
    if (_grid.activeCellsNb() > 0)
    {
        ...
        for (std::uint16_t j = 0; j < std::max(_grid._U.y(), _grid._V.y()); ++j)
        {
            ...
            {
                if (j < _grid._surface.y() && i < _grid._surface.x() && (id = _grid._pressureID(i,j,0)) > 0)
                {
                    _grid._pressure(i,j,0) = x(id-1);
                }
                if (j < _grid._U.y() && i < _grid._U.x() && _grid._U.label(i,j,0) & LIQUID)
                {
                    _grid._U(i,j,0) -= Config::N*(pressureAt(i,j,0,x,0) - pressureAt(i-1,j,0,x,0));
                }
                if (j < _grid._V.y() && i < _grid._V.x() && _grid._V.label(i,j,0) & LIQUID)
                {
                    _grid._V(i,j,0) -= Config::N*(pressureAt(i,j,0,x,0) - pressureAt(i,j-1,0,x,0));
                }
            }
        }
    }
...
void Project2D::preparePressureSolving(Eigen::SparseMatrix<double>& A, Eigen::VectorXd& b)
{
    A.resize(_grid.activeCellsNb(), _grid.activeCellsNb());
    A.data().squeeze();
...
inline double Project2D::div(const std::uint16_t i, const std::uint16_t j, const std::uint16_t k) const
{
    const double h = 1.0/Config::N;
    const double Udiv = _grid._U(i+1,j,k) - _grid._U(i,j,k);
    const double Vdiv = _grid._V(i,j+1,k) - _grid._V(i,j,k);
