...
{
public:
    explicit Field(U Xsize, U Ysize, U Zsize) : _Xsize(Xsize), _Ysize(Ysize), _Zsize(Zsize)
    {
        if (Config::dim == 2)
        {
            _Zsize = 1;
    ...
    const   T& operator()(const std::uint64_t idx)              const   { return _grid[idx]; }
            T& operator()(const U i, const U j, const U k)              { return _grid[idx(i,j,k)]; }
    const   T& operator()(const U i, const U j, const U k)      const   { return _grid[idx(i,j,k)]; }
    const   U& x()                                              const   { return _Xsize; }
    const   U& y()                                              const   { return _Ysize; }
    const   U& z()                                              const   { return _Zsize; }
    const   std::uint64_t& maxIt()                              const   { return _maxIt; }
    const   std::vector<T>& data()                              const   { return _grid; }
    const   CellLabel& label(const U i, const U j, const U k)   const   { return _label[idx(i,j,k)]; }
            CellLabel& label(const U i, const U j, const U k)           { return _label[idx(i,j,k)]; }
            CellLabel& label(const std::uint64_t idx)                   { return _label[idx]; }
            bool checked(const U i, const U j, const U k)       const   { return _label[idx(i,j,k)] & LIQUID || _label[idx(i,j,k)] & EXTRAPOLATED; }
            void reset()
            {
...
public:
    ...
    inline std::uint64_t hash(const std::uint16_t i, const std::uint16_t j, const std::uint16_t k) const
    {
        return (static_cast<std::uint64_t>(i) << 32) | (j << 16) | k;
    }
    inline const T getU(const R i, const R j, const R k, const std::uint8_t b) const
    {
        switch (b)
        {
            case 0:
